...
f1()});});function f3(){var Search=document.getElementById("Search");var person=prompt("开发者搜索支持,请输入搜索的内容","");debugger
if(person!=null){Search.href="https://kaifa.baidu.com/searchPage?wd="+person
...
function clearall(){var element=document.getElementById('canvas');context=element.getContext('2d');context.clearRect(0,0,S_width,S_height);}
...
drawpowder.prototype={init:function(e){var that=this;},handleEvent:function(e){var that=this;switch(e.type){case START_EVENT:that.touchStart(e);break;case MOVE_EVENT:that.touchMove(e);break;case END_EVENT:that.touchEnd(e);break;}},getPage:function(e,page){return isTouch?e.targetTouches[0][page]:e[page];},destroy:function(e){var that=this;that.element.removeEventListener(START_EVENT,that,false);document.removeEventListener(MOVE_EVENT,that,false);that.element.removeEventListener(END_EVENT,that,false);},touchStart:function(e){try{if(e.touches.length>1){e.stopPropagation();e.preventDefault();return;}}catch(e){}
...
var that=this;e.preventDefault();e.stopPropagation();},point:function(x,y,color){var that=this;context=that.element.getContext('2d');context.save();context.beginPath();context.arc(x,y,1,0,Math.PI*2,true);context.fillStyle=color;context.closePath();context.fill();context.restore();},spread:function(){var that=this;var time=0;context=that.element.getContext('2d');var spreadtimer=setInterval(function(){time++
