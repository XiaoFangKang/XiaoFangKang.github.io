...
_Blog.changeTitle=function(){var currentTitle=document.title;window.onblur=function(){document.title='谢谢您的关注！';}
...
$('.theme-switch').on('click',()=>{if(window.localStorage.getItem("effects")=="true"){window.localStorage&&window.localStorage.setItem('effects',"false")
history.go(0);}else{window.localStorage&&window.localStorage.setItem('effects',"true")
history.go(0);}})}
...
if(window.localStorage.getItem("effects")!="true"){f2()}
f1()});tableInit();});function tabOfcontentButton(){var button=document.getElementById("tab-of-content-button");var html=button.innerHTML;var tableOfContents=document.getElementById("TableOfContents");if("关闭"==html){button.innerHTML="开启";tableOfContents.style.visibility="hidden";}else{button.innerHTML="关闭";tableOfContents.style.visibility="visible";}}
function tableInit(){var button=document.getElementById("tab-of-content-button");try{var tableOfContents=document.getElementById("TableOfContents");var text=tableOfContents.innerText;if(text){button.style.visibility="visible";}else{button.style.visibility="hidden";}}catch(e){button.style.visibility="hidden";}}
function f3(){var Search=document.getElementById("Search");var person=prompt("开发者搜索支持,请输入搜索的内容","");if(person!=null){Search.href="https://kaifa.baidu.com/searchPage?wd="+person
Search.target="_blank"}else{Search.href="javascript:void(0)"
Search.target=""}}
function f(){var BACKGROUND_COLOR="rgb(255,255,255)";var POINT_NUM=99;var POINT_COLOR="rgba(0,0,0,0.7)";var LINE_LENGTH=10000;var cvs=document.createElement("canvas");cvs.width=window.innerWidth;cvs.height=window.innerHeight;cvs.style.cssText="\
    position:fixed;\
    top:0px;\
    left:0px;\
    z-index:-1;\
    opacity:1.0;\
    ";document.body.appendChild(cvs);var ctx=cvs.getContext("2d");var startTime=new Date().getTime();function randomInt(min,max){return Math.floor((max-min+1)*Math.random()+min);}
function randomFloat(min,max){return(max-min)*Math.random()+min;}
function Point(){this.x=randomFloat(0,cvs.width);this.y=randomFloat(0,cvs.height);var speed=randomFloat(0.3,1.4);var angle=randomFloat(0,2*Math.PI);this.dx=Math.sin(angle)*speed;this.dy=Math.cos(angle)*speed;this.r=1.2;this.color=POINT_COLOR;}
Point.prototype.move=function(){this.x+=this.dx;if(this.x<0){this.x=0;this.dx=-this.dx;}else if(this.x>cvs.width){this.x=cvs.width;this.dx=-this.dx;}
this.y+=this.dy;if(this.y<0){this.y=0;this.dy=-this.dy;}else if(this.y>cvs.height){this.y=cvs.height;this.dy=-this.dy;}}
Point.prototype.draw=function(){ctx.fillStyle=this.color;ctx.beginPath();ctx.arc(this.x,this.y,this.r,0,Math.PI*2);ctx.closePath();ctx.fill();}
var points=[];function initPoints(num){for(var i=0;i<num;++i){points.push(new Point());}}
var p0=new Point();p0.dx=p0.dy=0;var degree=2.5;document.onmousemove=function(ev){p0.x=ev.clientX;p0.y=ev.clientY;}
document.onmousedown=function(ev){degree=5.0;p0.x=ev.clientX;p0.y=ev.clientY;}
document.onmouseup=function(ev){degree=2.5;p0.x=ev.clientX;p0.y=ev.clientY;}
window.onmouseout=function(){p0.x=null;p0.y=null;}
function drawLine(p1,p2,deg){var dx=p1.x-p2.x;var dy=p1.y-p2.y;var dis2=dx*dx+dy*dy;if(dis2<2*LINE_LENGTH){if(dis2>LINE_LENGTH){if(p1===p0){p2.x+=dx*0.03;p2.y+=dy*0.03;}else return;}
var t=(1.05-dis2/LINE_LENGTH)*0.2*deg;ctx.strokeStyle="rgba(0,0,0,"+t+")";ctx.beginPath();ctx.lineWidth=1.5;ctx.moveTo(p1.x,p1.y);ctx.lineTo(p2.x,p2.y);ctx.closePath();ctx.stroke();}
return;}
function drawFrame(){cvs.width=window.innerWidth;cvs.height=window.innerHeight;ctx.fillStyle=BACKGROUND_COLOR;ctx.fillRect(0,0,cvs.width,cvs.height);var arr=(p0.x==null?points:[p0].concat(points));for(var i=0;i<arr.length;++i){for(var j=i+1;j<arr.length;++j){drawLine(arr[i],arr[j],1.0);}
arr[i].draw();arr[i].move();}
window.requestAnimationFrame(drawFrame);}
initPoints(POINT_NUM);drawFrame();}
function f1(){var a_idx=0;jQuery(document).ready(function($){$("body").click(function(e){var a=new Array("❤富强❤","❤民主❤","❤文明❤","❤和谐❤","❤自由❤","❤平等❤","❤公正❤","❤法治❤","❤爱国❤","❤敬业❤","❤诚信❤","❤友善❤");var $i=$("<span></span>").text(a[a_idx]);a_idx=(a_idx+1)%a.length;var x=e.pageX,y=e.pageY;$i.css({"z-index":999999999999999999999999999999999999999999999999999999999999999999999,"top":y-20,"left":x,"position":"absolute","font-weight":"bold","color":"rgb("+~~(255*Math.random())+","+~~(255*Math.random())+","+~~(255*Math.random())+")"});$("body").append($i);$i.animate({"top":y-180,"opacity":0},1500,function(){$i.remove();});});});}
function f2(){var S_width;var S_height;var isTouch='ontouchstart'in window,START_EVENT=isTouch?'touchstart':'mousedown',MOVE_EVENT=isTouch?'touchmove':'mousemove',END_EVENT=isTouch?'touchend':'mouseup';var powder=[];var man={x:30,y:110,step:0,type:'teststyle',speed:7,armlength:10,leglength:15,head:8,body:25};var isspread=false;var ispowder=true;var teststyle=[{armleft0:-45,armleft1:-65,armright0:45,armright1:25,legleft0:-45,legleft1:-65,legright0:45,legright1:25},{armleft0:-30,armleft1:-50,armright0:30,armright1:10,legleft0:-30,legleft1:-50,legright0:30,legright1:10},{armleft0:-15,armleft1:-35,armright0:15,armright1:-5,legleft0:-15,legleft1:-35,legright0:15,legright1:-5},{armleft0:0,armleft1:-20,armright0:0,armright1:-20,legleft0:0,legleft1:-20,legright0:0,legright1:-20},{armleft0:15,armleft1:-5,armright0:-15,armright1:-35,legleft0:15,legleft1:-5,legright0:-15,legright1:-35},{armleft0:30,armleft1:10,armright0:-30,armright1:-50,legleft0:30,legleft1:10,legright0:-30,legright1:-50}];var teststyle1=[{armleft0:-30,armleft1:0,armright0:30,armright1:0,legleft0:-5,legleft1:0,legright0:5,legright1:0},{armleft0:-30,armleft1:0,armright0:15,armright1:0,legleft0:-5,legleft1:0,legright0:45,legright1:0},{armleft0:-35,armleft1:0,armright0:15,armright1:0,legleft0:-5,legleft1:0,legright0:72,legright1:0},{armleft0:-35,armleft1:0,armright0:0,armright1:0,legleft0:-5,legleft1:0,legright0:97,legright1:81},{armleft0:-40,armleft1:0,armright0:-15,armright1:0,legleft0:-5,legleft1:0,legright0:111,legright1:108},{armleft0:-40,armleft1:0,armright0:-15,armright1:0,legleft0:-5,legleft1:0,legright0:135,legright1:135},{armleft0:-35,armleft1:0,armright0:0,armright1:0,legleft0:-5,legleft1:0,legright0:97,legright1:81},{armleft0:-30,armleft1:0,armright0:15,armright1:0,legleft0:-5,legleft1:0,legright0:45,legright1:0},{armleft0:-30,armleft1:0,armright0:30,armright1:0,legleft0:-5,legleft1:0,legright0:5,legright1:0},{armleft0:-51,armleft1:-27,armright0:51,armright1:41,legleft0:-22,legleft1:-18,legright0:22,legright1:18},{armleft0:-72,armleft1:-54,armright0:72,armright1:82,legleft0:-39,legleft1:-36,legright0:39,legright1:36},{armleft0:-93,armleft1:-81,armright0:93,armright1:123,legleft0:-56,legleft1:-54,legright0:56,legright1:54},{armleft0:-114,armleft1:-108,armright0:-114,armright1:164,legleft0:-73,legleft1:-72,legright0:73,legright1:72},{armleft0:-135,armleft1:-135,armright0:135,armright1:164,legleft0:-90,legleft1:-90,legright0:90,legright1:90},{armleft0:-135,armleft1:-135,armright0:135,armright1:164,legleft0:-90,legleft1:-90,legright0:90,legright1:90},{armleft0:-135,armleft1:-135,armright0:135,armright1:164,legleft0:-90,legleft1:-90,legright0:90,legright1:90},{armleft0:-114,armleft1:-108,armright0:-114,armright1:164,legleft0:-73,legleft1:-72,legright0:73,legright1:72},{armleft0:-72,armleft1:-54,armright0:72,armright1:82,legleft0:-39,legleft1:-36,legright0:39,legright1:36}];window.onload=function(){var cvs=document.createElement("canvas");S_width=window.innerWidth;S_height=window.innerHeight;cvs.width=window.innerWidth;cvs.height=window.innerHeight;cvs.id="canvas";cvs.style.cssText="\
        position:fixed;\
        top:0px;\
        left:0px;\
        z-index:-1;\
...
function clearall(){var element=document.getElementById('canvas');if(!element){return}
context=element.getContext('2d');context.clearRect(0,0,S_width,S_height);}
function unpowder(){clearall();ispowder=false;}
function dopowder(){clearall();ispowder=true;}
function drawpowder(id){var that=this;that.element=document.getElementById(id);}
drawpowder.prototype={init:function(e){var that=this;},handleEvent:function(e){var that=this;switch(e.type){case START_EVENT:that.touchStart(e);break;case MOVE_EVENT:that.touchMove(e);break;case END_EVENT:that.touchEnd(e);break;}},getPage:function(e,page){return isTouch?e.targetTouches[0][page]:e[page];},destroy:function(e){var that=this;if(!that.element){return}
that.element.removeEventListener(START_EVENT,that,false);document.removeEventListener(MOVE_EVENT,that,false);that.element.removeEventListener(END_EVENT,that,false);},touchStart:function(e){try{if(e.touches.length>1){e.stopPropagation();e.preventDefault();return;}}catch(e){}
var that=this;e.preventDefault();e.stopPropagation();if(isspread){return;}
isspread=true;downplacex=that.getPage(e,'clientX');downplacey=that.getPage(e,'clientY');for(var i=0;i<88;i++){var tempangle=Math.floor(Math.random()*360);var tempRadians=tempangle*Math.PI/180;var tempdistance=Math.floor(Math.random()*60);var tempcolor1=Math.floor(Math.random()*255);var tempcolor2=Math.floor(Math.random()*255);var tempcolor3=Math.floor(Math.random()*255);powder[i]={angle:tempangle,Radians:tempRadians,distance:tempdistance,color1:tempcolor1,color2:tempcolor2,color3:tempcolor3,hyaline:1};}
for(var j=0;j<powder.length;j++){var Px=Math.cos(powder[j].Radians)*powder[j].distance+downplacex-50;var Py=Math.sin(powder[j].Radians)*powder[j].distance+downplacey-50;that.point(Px,Py,"rgba("+powder[j].color1+","+powder[j].color2+","+powder[j].color3+","+powder[j].hyaline+")");}
that.spread();},touchMove:function(e){try{if(e.touches.length>1){e.stopPropagation();e.preventDefault();return;}}catch(e){}
var that=this;e.preventDefault();e.stopPropagation();},touchEnd:function(e){try{if(e.touches.length>1){e.preventDefault();return;}}catch(e){}
var that=this;e.preventDefault();e.stopPropagation();},point:function(x,y,color){var that=this;if(!that.element){return}
context=that.element.getContext('2d');context.save();context.beginPath();context.arc(x,y,1,0,Math.PI*2,true);context.fillStyle=color;context.closePath();context.fill();context.restore();},spread:function(){var that=this;var time=0;if(!that.element){return}
context=that.element.getContext('2d');var spreadtimer=setInterval(function(){time++
if(time>=100){clearInterval(spreadtimer);powder=[];if(ispowder){context.clearRect(0,0,S_width,S_height);}
isspread=false;return;}
if(ispowder){context.clearRect(0,0,S_width,S_height);}
for(var j=0;j<powder.length;j++){powder[j].distance=powder[j].distance+1;powder[j].hyaline=powder[j].hyaline-1/100;var Px=Math.cos(powder[j].Radians)*powder[j].distance+downplacex-50;var Py=Math.sin(powder[j].Radians)*powder[j].distance+downplacey-50;that.point(Px,Py,"rgba("+powder[j].color1+","+powder[j].color2+","+powder[j].color3+","+powder[j].hyaline+")");}},10)},man:function(x,y,step){var that=this;var mantype=eval(man.type)[man.step];var manx=x;var many=y;var legstart=man.body+man.head;var legleft0X=RetX(mantype.legleft0,man.leglength);var legleft0Y=RetY(mantype.legleft0,man.leglength);var legleft1Y=legleft0Y+RetY(mantype.legleft1,man.leglength);var legright0X=RetX(mantype.legright0,man.leglength);var legright0Y=RetY(mantype.legright0,man.leglength);var legright1Y=legright0Y+RetY(mantype.legright1,man.leglength);var armleft0X=RetX(mantype.armleft0,man.leglength);var armleft0Y=RetY(mantype.armleft0,man.leglength);var armright0X=RetX(mantype.armright0,man.leglength);var armright0Y=RetY(mantype.armright0,man.leglength);var groundleg=legleft1Y>legright1Y?"left":"right";if(groundleg=="left"){many=y-(man.leglength*2-legleft0Y-RetY(mantype.legleft1,man.leglength));}else{many=y-(man.leglength*2-legright0Y-RetY(mantype.legright1,man.leglength));}
if(!that.element){return}
context=that.element.getContext('2d');context.clearRect(0,0,S_width,S_height);context.save();context.beginPath();context.arc(manx,S_height-many,man.head,0,Math.PI*2,true);context.lineWidth=2;context.strokeStyle="#000";context.moveTo(manx,S_height-many+man.head);context.lineTo(manx,S_height-many+legstart);context.lineTo(manx+legleft0X,S_height-many+legstart+legleft0Y);context.lineTo(manx+legleft0X+RetX(mantype.legleft1,man.leglength),S_height-many+legstart+legleft1Y);context.moveTo(manx,S_height-many+legstart);context.lineTo(manx+legright0X,S_height-many+legstart+legright0Y);context.lineTo(manx+legright0X+RetX(mantype.legright1,man.leglength),S_height-many+legstart+legright1Y);context.moveTo(manx,S_height-many+legstart/2);context.lineTo(manx+armleft0X,S_height-many+legstart/2+armleft0Y);context.lineTo(manx+armleft0X+RetX(mantype.armleft1,man.armlength),S_height-many+legstart/2+armleft0Y+RetY(mantype.armleft1,man.armlength));context.moveTo(manx,S_height-many+legstart/2);context.lineTo(manx+armright0X,S_height-many+legstart/2+armright0Y);context.lineTo(manx+armright0X+RetX(mantype.armright1,man.armlength),S_height-many+legstart/2+armright0Y+RetY(mantype.armright1,man.armlength));context.stroke();context.closePath();context.restore();},manwalk:function(){var that=this;var tw=setInterval(function(){man.x=man.x+man.speed;man.step=(man.step+1)%(eval(man.type).length);if(man.x+20>=S_width){man.speed=-10;}
if(man.x-20<0){man.speed=10;}
that.man(man.x,man.y,man.step);},100)
setTimeout(function(){man={x:30,y:110,step:0,type:'teststyle',speed:7,armlength:10,leglength:15,head:8,body:25};},30000);}}
setTimeout(function(){new drawpowder('canvas').manwalk();},100);function RetX(Angle,length){return Math.sin(Angle*Math.PI/180)*length;}
function RetY(Angle,length){return Math.cos(Angle*Math.PI/180)*length;}};/*! lightgallery - v1.6.11 - 2018-05-22
